...
LEFT JOIN
    address a ON o.address_id = a.address_id
LEFT JOIN
    paymentstable p ON CAST(o.order_id AS VARCHAR) = p.merchant_order_id AND p.is_deleted = false
WHERE 
    1 = 1`;
    const values = [];

    if (user_id) {
      query += ` AND o.user_id = $${values.length + 1} AND o.is_deleted = false`;
      values.push(user_id);
    }
    if (search) {
      query += ` AND (u.full_name ILIKE $${values.length + 1} OR u.phone::text ILIKE $${values.length + 1} OR u.email ILIKE $${values.length + 1})`;
      values.push(`%${search}%`);
    }
    if (address_id) {
      query += ` AND o.address_id = $${values.length + 1}`;
    ...
    if (order_status) {
      if (order_status === "MY_ORDERS") {
        ...
        query += ` AND o.status = $${values.length + 1} AND o.is_deleted = false`;
        values.push(order_status);
      }
    }
    ...
    if (limit && page) {
      query += ` GROUP BY
      o.order_id, u.full_name, u.phone, u.email, a.state, a.pincode, a.city, a.full_address,
      p.payment_id, p.merchant_transaction_id, p.merchant_order_id, p.amount, p.currency, 
      p.status, p.callback_url, p.metadata, p.created_at
      ORDER BY o.created_at DESC
      LIMIT $${values.length + 1} OFFSET $${values.length + 2}`;
      values.push(limit);
      values.push((page - 1) * limit);
    ...
    console.error("Error in get_orders_model:", error);
    return {
      success: false,
      message: "An unexpected error occurred while fetching order data.",
      data: [],
